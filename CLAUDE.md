...
- **RTP-timestamp gap fill** (`WavWriter.writePacket`): silence (zero PCM) is inserted whenever the RTP timestamp advances more than the previous packet's sample count — bridging DTX silence-suppression gaps and lost packets. Without it the timeline is compressed and playback sounds accelerated. For G.711 the RTP timestamp unit is one 8 kHz sample, so deltas map 1:1 onto output samples. A single gap is capped at `maxGapSeconds` (30 s) to guard against garbage timestamps. **Limitation:** only the native G.711 `WavWriter` fills gaps; the `FfmpegWriter` (G.722/G.729) still concatenates payloads and can sound accelerated on lossy/DTX streams.
